...
@property (strong , nonatomic)NSArray * cateGoodS;
...
{
    if (indexPath.row<_cateGoodS.count) {
        NSDictionary * diction= [self.cateGoodS objectAtIndex:indexPath.row];
        if ([diction objectForKey:@"data"]) {
            _mainItem = [DCClassMianItem mj_objectArrayWithKeyValuesArray:[diction objectForKey:@"data"] ];
        }
    }
...
-(NSArray*)cateGoodS{
    ...
    if (!_cateGoodS) {
        _cateGoodS = [NSArray array];
    }
    return _cateGoodS ;
}
-(void)addGooditemArray:(NSArray*)array{
    
    self.cateGoodS = array ;
    _titleItem = [DCClassGoodsItem mj_objectArrayWithKeyValuesArray:array];
    _mainItem = [DCClassMianItem mj_objectArrayWithKeyValuesArray:[[array objectAtIndex:0] objectForKey:@"data"] ];
